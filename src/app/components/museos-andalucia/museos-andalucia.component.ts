...
export class MuseosAndaluciaComponent {
  ...
  buscar() {
    //let museosFiltrados = this.museos.filter(museo => museo.nombre.toLowerCase().includes("sevilla"));
    //const museosFiltrados = [].concat(this.museos);
    let textoBusqueda = (<HTMLInputElement>document.getElementById("input")).value;

    //console.log("INPUT --> " + (<HTMLInputElement>document.getElementById("input")).value);
